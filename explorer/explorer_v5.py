...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QMessageBox, QHeaderView, QGridLayout, QTextEdit, QProgressDialog, QLineEdit, QMenu
)
...
class DataExplorer(QMainWindow):
    def __init__(self, preload_file=None):
        ...
        # --- New controls for filtering genes by location ---
        self.locationLabel = QLabel("Location Filter:")
        control_layout.addWidget(self.locationLabel)
        
        self.armLineEdit = QLineEdit()
        self.armLineEdit.setPlaceholderText("Arm")
        control_layout.addWidget(self.armLineEdit)
        
        self.startLineEdit = QLineEdit()
        self.startLineEdit.setPlaceholderText("Start")
        control_layout.addWidget(self.startLineEdit)
        
        self.endLineEdit = QLineEdit()
        self.endLineEdit.setPlaceholderText("End")
        control_layout.addWidget(self.endLineEdit)
        
        self.filterButton = QPushButton("Filter Genes")
        self.filterButton.clicked.connect(self.filter_genes_by_location)
        control_layout.addWidget(self.filterButton)
        ...
        # Remove double click connection and set up custom context menu instead.
    ...
    def filter_by_row_location(self, row):
        ...
        # Extract arm, start, and end from the selected row.
        arm_val = str(row_data.get("arm", "")).strip()
        start_val = row_data.get("start", None)
        end_val = row_data.get("end", None)
        if arm_val == "" or start_val is None or end_val is None:
            QMessageBox.warning(self, "Filter Error", "The selected row does not have valid location information.")
            return
        filtered_df = self.df[self.df['arm'] == arm_val]
        filtered_df = filtered_df[(filtered_df['start'] >= start_val) & (filtered_df['end'] <= end_val)]
        if filtered_df.empty:
            QMessageBox.information(self, "No Results", "No genes found in the selected location range.")
            return
        self.model.updateDataFrame(filtered_df)
        # Also update the filter fields so the user sees what was applied.
        self.armLineEdit.setText(arm_val)
        self.startLineEdit.setText(str(start_val))
        self.endLineEdit.setText(str(end_val))
    
    def filter_genes_by_location(self):
        if self.df is None:
            return
        arm_val = self.armLineEdit.text().strip()
        try:
            start_val = int(self.startLineEdit.text().strip())
            end_val = int(self.endLineEdit.text().strip())
        except ValueError:
    ...
    def clear_filter(self):
        if self.df is None:
            return
        self.model.updateDataFrame(self.df)
        self.armLineEdit.clear()
